...
class CharacterApp:
    ...
    def save_character(self):
        if not self.selected_character:
            ...
            self.chat_log.insert(tk.END, "Chapter saved.\n")
        else:
            self.chat_log.insert(tk.END, "New Chapter Discoverd.\n")
        ...
        # Append the chat log content to the character's definition
        self.selected_character.definition += "\n" + self.chat_log.get("1.0", tk.END).strip()
        self.save_character_to_file(self.selected_character)
    ...
    def send_message(self):
        ...
        # Check if the response contains a page number reference
        if "**page" in ai_response.lower():
            try:
                page_number = ai_response.split("**page")[1].split("**")[0].strip()
                if f"**page {page_number}**" not in self.selected_character.definition.lower():
                    # Update definition with new page content
                    new_page_content = f"**Page {page_number}**\n" + self.generate_new_page_content(page_number)
                    self.selected_character.definition += new_page_content
                    # Update the character file
                    self.save_character_to_file(self.selected_character)
            except IndexError:
                self.chat_log.insert(tk.END, "I'm sorry, it seems like there was a mistake in the page number. Let's continue your adventure:\n")
    ...
    def generate_new_page_content(self, page_number):
        # Generate new content for the page
        new_page_prompt = [
            {"role": "system", "content": "Generate new page content for a choose-your-own-adventure story."},
            {"role": "user", "content": f"Please create new content for **page {page_number}** in the story."}
        ]
        new_page_content = generate_response(new_page_prompt)
